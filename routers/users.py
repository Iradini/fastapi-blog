...
from schemas import PaginatedPostsResponse, PostResponse, Token, UserCreate, UserPrivate, UserPublic, UserUpdate
...
async def loging_for_access_token(
    ...
    # Note: OAuth2PasswordRequestForm uses "username" field, but we treated as email
    result = await db.execute(
        select(models.User).where(
